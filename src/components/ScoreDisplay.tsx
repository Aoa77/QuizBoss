...
export default function ScoreDisplay(context: AppContext) {
    ...
    return (
        <section id={scoreArea.target} ref={scoreArea.object} className="score hidden">
            <div>{quizData?.scoreText}</div>
            <div>{quizData?.bestText}</div>
            <div className="row">
                <div id={scoreValue.target} ref={scoreValue.object}>{score}</div>
                <div id={bestValue.target} ref={bestValue.object}>{best}</div>
            </div>
        </section>
    );
}
